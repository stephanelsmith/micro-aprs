...
def get_sin_table(size   = 1024, 
    ...
    if square:
        # return a square wave instead of sin
        if signed:
            return array('h', (ampli if int(ampli*math.sin(x))>0 else -ampli for x in frange(0,2*math.pi,2*math.pi/size)))
        else:
            return array('h', (0x7fff+ampli if int(ampli*math.sin(x)) > 0 else 0x7fff-ampli for x in frange(0,2*math.pi,2*math.pi/size)))
    else:
        if signed:
...
def create_afsk(fs     = 22050, # sampling frequency
    ...
    # sintbl is an array
    sintbl = get_sin_table(size    = _SINTBL_SIZE,
                            signed  = signed,
                            ampli   = ampli,
                            )

    # foreach afsk frequency, compute the integer step size
    stepint = array('H', (int(_SINTBL_SIZE/(fs/f)) for f in afsks))
    # foreach afsk frequency, compute the residual step size
    stepres = array('H', (int(((_SINTBL_SIZE/(fs/f))%1)*_RESIDUE_SIZE) for f in afsks))
    ...
    if IS_UPY and HAS_VIPER:
        ...
        def inner(k:int)->int:
            ...
            _phaseint:int = int(phaseint)
            _phaseres:int = int(phaseres)
            _signed:int = int(signed)
            _stepint = ptr16(stepint) # indexing ALWAYS return uint
            _stepres = ptr16(stepres) # indexing ALWAYS return uint
            _sintbl  = ptr16(sintbl)  # indexing ALWAYS return uint
           
            if _signed:
                _phaseint += int(utoi16(_stepint[k]))
                _phaseres += int(utoi16(_stepres[k]))
                _phaseint += _phaseres // _RESIDUE_SIZE
                _phaseres %= _RESIDUE_SIZE
                _o:int = int(utoi16(_sintbl[_phaseint % _SINTBL_SIZE]))
            else:
                _phaseint += _stepint[k] 
                _phaseres += _stepres[k] 
                _phaseint += _phaseres // _RESIDUE_SIZE
                _phaseres %= _RESIDUE_SIZE
                _o:int = _sintbl[_phaseint%_SINTBL_SIZE]

            phaseint = (_phaseint<<1)|1 # INTEGER TO OBJECT HACK
            phaseres = (_phaseres<<1)|1 # INTEGER TO OBJECT HACK
            return _o
        return inner
    else:
        def inner(k:int)->int:
            ...
            phaseres %= _RESIDUE_SIZE
            o:int = sintbl[phaseint % _SINTBL_SIZE]
            return o
        return inner

# create a tone generator, samples in baud increments
def create_afsk_tone_gen(fs     = 22050,        # sampling frequency
                         afsks  = [2200, 1200], # space (0) and mark (1)
                         signed = True,
                         ampli  = 0x7fff,
                         baud   = 1200,         # baud rate of tones
                         ):
    stepint:int  = int(fs/baud)
    stepres:int  = int(((fs/baud)%1)*_RESIDUE_SIZE)
    phaseint:int = 0
    baudidx:int  = 0

    # create afsk
    afsk = create_afsk(fs     = fs,
                       afsks  = afsks,
                       signed = signed,
                       ampli  = ampli,
                       )
    def inner(k:int)->int:
        nonlocal phaseint, baudidx
        baudidx += 1 # every call, increment

        # determine the phase we want to advance to using the current baudidx
        stopphase:int = (baudidx*stepint) + ((baudidx*stepres)//_RESIDUE_SIZE)

        while phaseint < stopphase:
            phaseint += 1 # increment the phase
            yield afsk(k) # output
    return inner
